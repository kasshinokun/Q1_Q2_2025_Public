...
public class FileReader {//Leitor de Arquivo
  ...
  //Use o Eclipse para Rodar por Favor    
    public static ArrayList<Object_X> read_file2(String pathFile,int condition){
	    try {
	    	if (condition >=1&&condition<=4){//leitura, atribuição e escrita mediante escolha
		        ...
		        String linha;
		        char c = '.'; 
		        ArrayList<Object_X> list_person= new ArrayList();
		        for (int i =1;(linha = reader.readLine()) != null;i++) {
		            if(linha.indexOf(c) == -1&&i!=1) {
		                
		            	
		                String[] result=linha.split(","); 
		               //FileExplorer.Splitter_String(linha, i,k);
		                print("Linha 0"+(i-1)+": "+result[0]+" "+result[1]+" "+extract_date(result[1]));//Exibe "Linha 012444: ???        33092219940724001X"
		                list_person.add(new Object_X(result[0],result[1]));
		            }else {
		                System.out.println("Não execute 0"+i+": "+linha);
		            }    
		        }
		        if (condition==1) {//Apenas para exibir o resultado da leitura
		        	System.out.println("Exibindo Lista.\n");
		        	for(Object_X y:list_person) {
			        	print(y.printObject());
			        }
		        	//print(pathFile);
		        }else{
		        	
		        	return list_person;//retorno preparativo para escritor e seletor
		        }
    ...
    */
    ...
    public static String extract_date(String id_chinese){
    	//33092219940724001X
    	String date=id_chinese.substring( 6,  14);
    	
    	return date.substring(6, 8)+"-"+date.substring(4, 6)+"-"+date.substring(0, 4);
    	
    }
    public static Calendar extract_date2(String id_chinese){
    	...
    	try {
		    ...
		    java.util.Date strdate = (java.util.Date)new SimpleDateFormat(pattern).parse(stringdate);
