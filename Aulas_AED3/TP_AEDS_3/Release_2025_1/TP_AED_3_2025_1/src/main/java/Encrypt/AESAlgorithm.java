 ...
 *Source-URL: https://gist.github.com/asika32764/224c176d89289b80e318
...
public class AESAlgorithm {
    ...
    private byte[][] addRoundKey(byte state[][], int w[], int l) {
        ...
        for (int c = 0; c < Nb; c++) {
        	for(int i=0;i<4;i++){
        		stateNew[i][c] = (byte) (state[i][c] ^ getByte(w[l + c], 4-(i+1)));
        	}
        }
        return stateNew;
    }
    ...
    public byte[] createKey() {
        ...
        System.out.println(key.length);//Feedback
        return key;
    }
    
    public byte[] createKey(String secret) {
    	int size=4 * Nk;
