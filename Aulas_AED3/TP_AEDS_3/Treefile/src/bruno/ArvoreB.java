...
public class ArvoreB {
	...
	public ArvoreB() throws IOException {
		this("btree.index", (int)10);
	}
	
	public ArvoreB(String fileName, int ordem) throws IOException {
		this.ordem = ordem;
	...
	private KeyAddressPair inserir(int chave, long endereco, long endPag) throws Exception {
		...
		if(paginaAtual.getN() < this.ordem - 1) {
			...
			paginaAtual.setN((int) (paginaAtual.getN()/2));
			...
			if(this.ordem % 2 == 0) {
				paginaIrma.setN((int) (paginaAtual.getN() + 1));
			}
			...
			//Para esse caso, a chave a ser promovida é justamente a que queremos inserir, portanto ela é simplesmente retornada para ser promovida, sendo que o seu filho à direita será a nova página criada
				...
				paginaIrma.setN((int)(paginaIrma.getN() - 1));
				...
				paginaAtual.setN((int)(paginaAtual.getN() - 1));
				inserirOrdenado(pair, paginaAtual);
			}
	...
	private long remove(int key, long endPagAtual, long endPagPai) throws Exception {
		...
		if(!pagAtual.isLeaf()) {
			...
			for(int i=0; i<pagAtual.getN(); i++) {
				...
				if(key == aux.getKey()) {
					resp = aux.getAddress();
					KeyAddressPair antecessor = getAntecessor(pagAtual, (int)i);
					long[] addresses = new long[2];
					addresses = getEndAntecessor(pagAtual, endPagAtual, (int)i);
					//O array addresses recebe os endereços da página folha onde está o antecessor e do pai dessa, para que o antecessor seja removido da folha
					
					remove(antecessor.getKey(), addresses[0], addresses[1]);
			...
			if(!exists) {
				...
				if(pagAtual.getN() > this.ordem / 2) {
					...
					if(siblingIndex == 1) {
						...
						long siblingAddress = pagPai.getSonAtIndexOf(siblingIndex);
						aux = stealBiggest(siblingAddress);
						...
						long siblingAddress = pagPai.getSonAtIndexOf(siblingIndex);
						aux = stealintest(siblingAddress);
						...
						//A página irmã que cedeu sua maior chave já foi atualizada no arquivo por meio do método stealintest()
	...
	private int getSonIndex(long pagAddress, Pagina pagPai) throws Exception{
		int i;
		for(i=0; i<pagPai.getN() + 1 && pagPai.getSonAtIndexOf(i) != pagAddress ; i++);
		return i;
	}
	
	private KeyAddressPair stealintest(long pagAddress) throws Exception{
		byte[] baPagAtual = new byte[(20 * ordem) - 8];
		indexFile.seek(pagAddress);
		indexFile.read(baPagAtual);
		Pagina pagAtual = new Pagina(baPagAtual, this.ordem);
	...
	private KeyAddressPair stealBiggest(long pageAddress) throws Exception {
		//retira o par chave/endereço de maior chave
		
		byte[] baPag = new byte[(20 * ordem) - 8];
		indexFile.seek(pageAddress);
		...
		pagAtual.setN((int)(pagAtual.getN() - 1));
	...
	private KeyAddressPair getAntecessor(Pagina pagAtual, int pairIndex) throws Exception {
	...
	private long[] getEndAntecessor(Pagina pagAtual, long iniAddress, int pairIndex) throws Exception {
		byte[] ba = new byte[(20 * ordem) - 8];
		long parentAddress;
