...
import math # For ceil in page calculation
...
class TrafficAccidentsDB:
    ...
    def get_total_valid_records_count(self) -> int:
        """
        Counts the total number of valid records in the database.
        ...
        try:
            ...
            with open(self.db_path, 'rb') as f:
                ...
                while True:
                    ...
                    record_data_raw = None
                    try:
                        record_data_raw = self._read_next_record_raw(f)
                    except DatabaseError as e:
                        logger.warning(f"Skipping corrupt record at position {current_pos_before_read} during valid count: {str(e)}")
                        # Attempt to seek past the problematic part.
                        f.seek(current_pos_before_read + CHUNK_SIZE) 
                        continue
                    except Exception as e:
                        logger.error(f"Unexpected error counting valid records at {current_pos_before_read}: {traceback.format_exc()}")
                        break # Stop on unexpected error
                        
                    if record_data_raw is None: # End of file
                        break
                    
                    if record_data_raw['validation']:
                        count += 1
            logger.info(f"Total valid records counted: {count}.")
            return count
        except FileLockError:
            raise
        except FileNotFoundError:
            ...
            logger.error(f"Error getting total valid records count: {traceback.format_exc()}")
            raise DatabaseError(f"Failed to count valid records: {str(e)}")
        finally:
            self._release_lock()

    def read_records_paginated(self, offset: int = 0, limit: Optional[int] = None, only_valid: bool = False) -> List[Dict[str, Any]]:
        """
        Reads records from the database file with pagination.
        Includes error recovery for individual corrupt records.
        Can optionally filter to return only valid records.
        Returns raw record data (with 'data_bytes').
        """
        records = []
        records_read_count = 0
        records_skipped_due_to_offset = 0
        ...
        try:
            ...
            with open(self.db_path, 'rb') as f:
                ...
                while True:
                    ...
                    try: 
                        ...
                        logger.warning(f"Skipping corrupt record at position {current_pos_before_read} during paginated read: {str(e)}")
                        f.seek(current_pos_before_read + CHUNK_SIZE) 
                        continue
                    except Exception as e:
                        logger.error(f"Unexpected error when reading raw record for pagination at position {current_pos_before_read}: {traceback.format_exc()}")
                        break 
                        
                    if record_data_raw is None: # End of file
                        break
                    
                    # Apply 'only_valid' filter immediately after reading raw data
                    if only_valid and not record_data_raw['validation']:
                        continue # Skip this record if it's invalid and only_valid is True

                    # Apply offset (0-based index) for the *filtered* records
                    if records_skipped_due_to_offset < offset:
                        records_skipped_due_to_offset += 1
                        continue
            ...
            logger.info(f"Successfully read {len(records)} raw records from the database (offset: {offset}, limit: {limit}, only_valid: {only_valid}).")
            return records
        except FileLockError:
            raise
        except FileNotFoundError:
        ...
        try:
            ...
            with open(self.db_path, 'rb') as f:
                ...
                while True:
                    ...
                    try: # Added try-except block here for _read_next_record_raw
                        record_data_raw = self._read_next_record_raw(f)
                    except DatabaseError as e:
                        logger.warning(f"Skipping corrupt record at position {current_pos_before_read} during ID search: {str(e)}")
                        f.seek(current_pos_before_read + CHUNK_SIZE) # Try to jump past the corrupt record
    ...
    def _read_next_record_raw(self, file_obj) -> Optional[Dict[str, Any]]:
        ...
        Raises DatabaseError for structural corruption or invalid data size.
        """
        try:
            ...
            # Modified to explicitly handle data_size == 0 as an invalid case
            if not (data_size > 0 and data_size <= 10 * 1024 * 1024): # Max 10MB per record
                logger.warning(f"Invalid record data size ({data_size} bytes) for record ID {record_id} at offset {file_obj.tell() - 9}. Skipping this record.")
                # Attempt to seek past the problematic data. Add data_size to current pos.
                # If data_size was 0, it means we don't know how far to seek, so a heuristic is needed.
                # For `data_size == 0`, we assume it's completely corrupted and try to jump to the next potential record start.
                if data_size == 0:
                    # Heuristic: move past where the expected data should have been, assuming a minimal record size.
                    # This is imperfect but prevents infinite loops on zero-sized records.
                    file_obj.seek(file_obj.tell() + 100) # Jump 100 bytes forward as a guess
                else:
                    file_obj.seek(file_obj.tell() + data_size) # Jump to the end of the problematic data
                return None # Indicate that this record should be skipped
...
def setup_ui():
    ...
    try:
        ...
        # Initialize session state variables for redirection and filter reset
        if 'redirect_to_operation' not in st.session_state:
            st.session_state.redirect_to_operation = None
        if 'reset_view_all_filters' not in st.session_state:
            st.session_state.reset_view_all_filters = False
        ...
        # Process redirection signal *before* the radio widget is rendered
        if st.session_state.redirect_to_operation:
            ...
            # If redirecting to "View All Records", also set the reset flag
            if st.session_state.redirect_to_operation == "📄 View All Records":
                st.session_state.reset_view_all_filters = True
...
def view_all_records(db: TrafficAccidentsDB):
    ...
    if 'search_crash_type_all' not in st.session_state:
        st.session_state.search_crash_type_all = ""
    if 'min_injuries_all' not in st.session_state:
        st.session_state.min_injuries_all = 0.0
    if 'only_valid_checkbox' not in st.session_state:
        st.session_state.only_valid_checkbox = False
    if 'view_page_size' not in st.session_state:
        st.session_state.view_page_size = 20
    if 'view_page_num' not in st.session_state:
        st.session_state.view_page_num = 1

    # Check and reset filters if navigation occurred
    if st.session_state.reset_view_all_filters:
        st.session_state.search_crash_type_all = ""
        st.session_state.min_injuries_all = 0.0
        st.session_state.only_valid_checkbox = True  # Default to showing only valid records
        st.session_state.view_page_size = 20         # Default to 20 records per page
        st.session_state.view_page_num = 1            # Default to page 1
        st.session_state.reset_view_all_filters = False
        st.rerun() # Rerun to apply the reset values
    ...
    with st.expander("🔍 Filter & Sort Records", expanded=True):
        ...
        with filter_cols[1]:
            min_injuries = st.number_input("Min Total Injuries", min_value=0.0, value=st.session_state.min_injuries_all, step=0.1, key="min_injuries_all", help="Only show records with at least this many total injuries.")
        with filter_cols[2]:
            only_valid = st.checkbox("Show Only Valid Records", value=st.session_state.only_valid_checkbox, key="only_valid_checkbox", help="Toggle to display only records marked as valid.")
        
    # Get total records for pagination based on the 'only_valid' filter
    if only_valid:
        total_records_for_pagination = db.get_total_valid_records_count()
    else:
        total_records_for_pagination = db.get_last_id() # This is the total number of records, valid or invalid

    if total_records_for_pagination == 0:
        st.info("No records found in the database matching the current filters. Try importing from CSV or adjusting filters.")
        return

    # Pagination controls - use direct session state for default values
    page_cols = st.columns([1, 1, 2])
    
    with page_cols[0]:
        # Use a temporary variable for the selectbox value, and then update session state if changed
        selected_page_size = st.selectbox(
            "Records per page",
            [10, 20, 50, 100, total_records_for_pagination], # Option to show all records if applicable
            index=[10, 20, 50, 100, total_records_for_pagination].index(st.session_state.view_page_size) 
            if st.session_state.view_page_size in [10, 20, 50, 100, total_records_for_pagination] else 1, # Default to 20 per page, persist
            key="page_size_widget"
        )
        # Update session state only if the widget value is different from the stored value
        if st.session_state.view_page_size != selected_page_size:
            st.session_state.view_page_size = selected_page_size
            st.session_state.view_page_num = 1 # Reset to page 1 when page size changes
            st.rerun() # Rerun to apply new page size and reset page number
        
        # Ensure page_size used for calculations is the one from session state
        page_size = st.session_state.view_page_size

    # Calculate total pages based on the filtered total records
    total_pages = max(1, math.ceil(total_records_for_pagination / page_size)) 
    ...
    with page_cols[1]:
        ...
        if st.session_state.view_page_num > total_pages:
            st.session_state.view_page_num = total_pages # Adjust if current page is now out of bounds

        selected_page_num = st.selectbox(
            "Page", 
            options=page_options,
            index=st.session_state.view_page_num - 1, # Adjust for 0-indexed selectbox
            key="page_num_widget"
        )
        # Update session state only if the widget value is different from the stored value
        if st.session_state.view_page_num != selected_page_num:
            st.session_state.view_page_num = selected_page_num
            st.rerun() # Rerun to apply new page number

    # Calculate offset for fetching records
    start_offset = (st.session_state.view_page_num - 1) * page_size
    
    # Fetch records for the current page, applying validation filter at the source
    current_page_raw_records = []
    with st.spinner(f"Loading records for page {st.session_state.view_page_num}..."):
        try:
            current_page_raw_records = db.read_records_paginated(
                offset=start_offset, 
                limit=page_size, 
                only_valid=only_valid # Pass the filter to the database read function
            )
            logger.info(f"Loaded {len(current_page_raw_records)} raw records for page {st.session_state.view_page_num} (only_valid: {only_valid}).")
        except DatabaseError as e:
    ...
    # Further filter records based on the fetched subset (crash type and injuries)
    # This step is still necessary because search_query and min_injuries filters are applied here.
    filtered_records_with_obj = [] 
    for record_raw in current_page_raw_records:
        try:
            ...
            logger.warning(f"Skipping record ID {record_raw.get('id', 'N/A')} due to deserialization/validation error during UI filtering: {e}")
            continue 
        except Exception as e:
            logger.error(f"Unexpected error processing raw record for display (ID: {record_raw.get('id', 'N/A')}): {traceback.format_exc()}")
            continue

    st.info(f"Displaying {len(filtered_records_with_obj)} record(s) on page {st.session_state.view_page_num} (of {total_pages} pages) after filters.")
    ...
    for i, record_with_obj in enumerate(filtered_records_with_obj):
        ...
        with st.expander(
            ...
            with tab1:
                display_record_data(record_with_obj['data']) 
            with tab2:
    ...
    st.caption(f"Showing records {start_offset + 1}-{start_offset + len(filtered_records_with_obj)} of {total_records_for_pagination} (Total matching filters)")
