        ...
        try:
            ...
            with open(self.db_path, 'rb') as f:
                ...
                while True:
                    ...
                    try: # Added try-except block here to catch errors from _read_next_record_raw
                        ...
                        # Attempt to seek past the problematic part. This is a heuristic.
                        # A more robust solution might involve re-scanning from a known good point.
                        ...
                        break # Stop reading on unexpected error
...
def view_all_records(db: TrafficAccidentsDB):
    ...
    # Pagination controls - modify to use callback pattern
    page_cols = st.columns([1, 1, 2])
    
    with page_cols[0]:
        def update_page_size():
            st.session_state.view_page_size = st.session_state.page_size_widget
            st.session_state.view_page_num = 1  # Reset to first page when page size changes
            
        page_size = st.selectbox(
            "Records per page",
            [10, 20, 50, 100, total_records_in_db],
            index=[10, 20, 50, 100, total_records_in_db].index(st.session_state.view_page_size) 
            if st.session_state.view_page_size in [10, 20, 50, 100, total_records_in_db] else 1,
    ...
    with page_cols[1]:
        current_page = st.selectbox(
            ...
            index=min(st.session_state.view_page_num, total_pages) - 1,
            key="page_num_widget"
        )
        st.session_state.view_page_num = current_page  # Update session state
