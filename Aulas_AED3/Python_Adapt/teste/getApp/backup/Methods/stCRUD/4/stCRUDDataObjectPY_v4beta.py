...
import tempfile
import traceback
import uuid # For stable record IDs if needed, though not used in final code for content-based ID

# --- Configuration Constants (Centralized) ---
# Using a separate config dict for better management
APP_CONFIG = {
...
# --- Data Structure ---
# - record_id: unique identifier (SHA256 hash of data content)
# - timestamp: datetime of creation/last update (float)
# - data_hash: SHA256 hash of the data content (redundant if record_id is data hash, but useful for integrity checks if record_id was a UUID)
                        ...
                        if "inserted" in line or "updated" in line or "deleted" in line or "imported" in line or "rebuilt" in line:
                            try:
                                ...
                                if len(parts) >= 4:
                                    ...
                                    if len(update_entries) >= APP_CONFIG["MAX_LOG_ENTRIES_DISPLAY"]:
                                        break
                            except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        Path(APP_CONFIG["BACKUP_DIR_NAME"]).mkdir(parents=True, exist_ok=True) # Ensure backup dir exists
    except OSError as e:
