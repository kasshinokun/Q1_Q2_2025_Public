...
import traceback
import hashlib
import fcntl # For OS-level file locking
...
MAX_FILE_SIZE_MB = 10 # Maximum CSV file size for import
...
class DataObject:
    ...
    def _validate_date(date_str: Optional[str]) -> str:
        ...
        Handles various common input formats.
        """
        if not date_str:
        ...
        logger.warning(f"Invalid or unparseable date format for '{date_str}'. Returning empty string.")
        return "" # If no format matches, return empty string
...
class TrafficAccidentsDB:
    ...
    def _acquire_lock(self, timeout: int = 10, interval: float = 0.5):
        ...
        Implements a timeout and retry mechanism.
        """
        ...
        while True:
            try:
                ...
                if time.time() - start_time > timeout:
                    raise FileLockError(f"Failed to acquire file lock on {self.lock_file_path} within {timeout} seconds.")
                logger.warning(f"Lock file busy, retrying in {interval} seconds...")
                time.sleep(interval)
            except Exception as e:
    ...
    def _release_lock(self):
        ...
        if hasattr(self, '_lock_file_handle') and self._lock_file_handle:
            try:
                ...
                self._lock_file_handle = None # Clear the handle
        ...
        try:
            ...
            with open(self.db_path, 'rb') as f:
                ...
                while True:
                    current_pos = f.tell()
                    try:
                        record_data = self._read_next_record_raw(f)
                        if record_data is None: # End of file
                            break
                        
                        # Deserialize and validate the DataObject
                        data_obj = DataObject.from_bytes(record_data['data_bytes'])
                        
                        records.append({
                            'id': record_data['id'],
                            'validation': record_data['validation'],
                        ...
                        # If a record is corrupt, try to skip past it
                        # This simple recovery might not always work if the corruption is severe,
                        # but it's a best effort to continue reading.
                        ...
                        # Fatal error for current read, cannot continue safely
                        break 
        
            logger.info(f"Successfully read {len(records)} records from the database.")
            return records
        except FileLockError:
            raise
        except FileNotFoundError:
            ...
            with open(self.db_path, 'rb') as f:
                ...
                while True:
                    ...
                    try:
                        ...
                        if record_data['id'] == search_id:
                            # Deserialize and validate the DataObject for the found record
                            data_obj = DataObject.from_bytes(record_data['data_bytes'])
                            return {
                                'id': record_data['id'],
                                'validation': record_data['validation'],
                                ...
                                'checksum': record_data['checksum'],
                                'data': data_obj,
                                'position': current_pos # Position of the record header
                            }
                    except (DatabaseError, DataValidationError) as e:
...
def display_record_data(data_obj: DataObject):
    ...
    if not isinstance(data_obj, DataObject):
        st.error("Invalid data object provided for display.")
...
def view_all_records(db: TrafficAccidentsDB):
    ...
    if 'all_records' not in st.session_state or st.session_state.all_records is None:
        st.session_state.all_records = []
        with st.spinner("Loading records from database... This might take a moment for large databases."):
            try:
                st.session_state.all_records = db.read_all_records()
                st.success(f"Successfully loaded {len(st.session_state.all_records)} records.")
            except DatabaseError as e:
    ...
    with st.expander("🔍 Filter & Sort Records", expanded=True):
        ...
        for record in records:
            ...
            if only_valid and not record.get('validation', True): # Default to True if 'validation' key missing (old record format)
                continue
            
            data_obj = record['data'] # Get the DataObject instance
    ...
    st.info(f"Displaying {len(filtered_records)} record(s) after filtering.")
    ...
    with page_cols[0]:
        page_size = st.selectbox(
            "Records per page",
            [10, 20, 50, 100, len(filtered_records)], # Option to show all
            index=1,
            key="view_page_size"
        )
    
    total_pages = max(1, (len(filtered_records) + page_size - 1) // page_size) # Ceiling division
    with page_cols[1]:
    ...
    end_idx = min(start_idx + page_size, len(filtered_records))
    
    records_to_display = filtered_records[start_idx:end_idx]
    ...
    for i, record in enumerate(records_to_display):
        ...
        expander_key = f"record_expander_{record['id']}"
        with st.expander(
            f"📝 Record ID: {record['id']} "
            f"({'✅ Valid' if record['validation'] else '❌ Invalid'}) "
            f"- {getattr(record['data'], 'crash_date', 'N/A')} - {getattr(record['data'], 'crash_type', 'N/A')}",
            ...
            with tab2:
                ...
                record_data_dict = {field: getattr(record['data'], field, None) for field in FIELDS}
                st.json(record_data_dict)
            with tab3:
                st.markdown(f"**Record Size (bytes):** `{record['size']}`")
                st.markdown(f"**MD5 Checksum:** `{record['checksum']}`")
                st.info("Checksum helps verify data integrity. Mismatches could indicate corruption.")
    
    st.caption(f"Showing records {start_idx + 1}-{end_idx} of {len(filtered_records)} (Total: {len(records)})")
...
def search_by_id(db: TrafficAccidentsDB):
        ...
        if search_button_clicked:
            if search_id_input > last_id and last_id > 0:
                ...
                with st.spinner(f"Searching for record ID {search_id_input}..."):
                    try:
                        record = db.read_record_by_id(search_id_input)
                        st.session_state.search_result_id = record
                        st.session_state.search_error_id = None if record else f"Record ID {search_id_input} not found in the database."
    ...
    with col2:
        if st.session_state.search_result_id:
            record = st.session_state.search_result_id
    ...
    with st.form("record_form", clear_on_submit=True):
        ...
        with cols1[2]:
            ...
            damage = st.text_input(
                "Damage Description",
                help="Brief description of property damage.",
                key="add_damage"
            )
        ...
        if submitted:
            ...
            if not crash_date:
                ...
                try:
                    ...
                    row_data = [""] * len(FIELDS)
                    row_data[0] = crash_date.strftime('%Y-%m-%d')
